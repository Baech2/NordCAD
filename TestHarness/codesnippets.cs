...
foreach (var parentProperty in parentProperties)
{
    ...
    if (parentProperty.Name == currentAttribute.ParentPropertyName)
    {
        ...
        //X og Y i OrCad/Capture er type long, så derfor skal string results parses til double for derefter at convert det til en long. + brug af format som er defineret før Switch.                                    
...
foreach (var parentProperty in parentProperties)
{
    ...
    if (parentProperty.Name == currentAttribute.ParentPropertyName)
    {
        ...
        //NamedPartX1 bliver "lagt" i parentPropertyValue som senere bliver brugt til SetValue for x1 & y1 på childProperty (capture).
